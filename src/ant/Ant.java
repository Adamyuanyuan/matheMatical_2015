...
//        p = calculateP1(pheromone);
    ...
    private double[] calculateP1(double[][] pheromone) {
        ...
        for (int i = 0; i < cityNum; i++) {
            ...
            // // 先检验 在不在五个的窗口里面，不在的话，概率为0，在的话，按上面计算的概率值
            // if (i == j.intValue()) {
 ...
 // 计算下一个转移数组的概率，输入信息素，对返回西安概率设定一个固定的概率
    ...
    private void updateStatus(int selectP) throws IOException {
        ...
        // 计算修改后的时间，日期，等信息
        calculateNeedDateAndChange(currentTime, selectP);
    ...
    private void calculateNeedDateAndChange(int curruntTime, int selectP) throws IOException {
        double needDate = 0;
